...
class NERModel(LanguageModel):
    ...
    def create_feed_dict(self, input_batch, dropout, label_batch=Nome):
        feed_dict = {
    ...
    def __init__(self, config):
        ...
        ont_hot_prediction = tf.argmax(self.predictions, 1)
        correct_prediction = tf.equal(tf.argmax(self.labels_placeholder,1), ont_hot_prediction)
    ...
    def run_epoch(self, session, input_data, input_labels, shuffle=True, verbose=True):
        ...
        tatal_steps = len(orig_X) / self.config.batch_size
    ...
    with tf.Graph().as_default():
        ...
        with tf.Session() as session:
            ...
            for epoch in range(config.max_epochs):
                ...
                start = time.time()
                train_loss, train_ass = model.run_epoch(session, model.X_train, model.y_train)
                val_loss, predictions = model.predict(session, model.X_dev, model.y_dev)
                print("Traning loss:{}".format(train_loss))
